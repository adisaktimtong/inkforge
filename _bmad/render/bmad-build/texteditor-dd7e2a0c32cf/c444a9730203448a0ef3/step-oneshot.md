...
If `{story_key}` is not empty and `/Users/adisak.t/Repo/texteditor/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `/Users/adisak.t/Repo/texteditor/_bmad/render/bmad-build/texteditor-dd7e2a0c32cf/c444a9730203448a0ef3/sync-sprint-status.md` with `{target_status}` = `in-progress`.
...
Write what triggered the stop in `## Implementation Notes`. Then update `{spec_file}`: add back `## Code Map` (filled in from what you learned while implementing) and `## Open Questions` (one question per gap), set `route: 'full'` and `status: 'draft'`. Go back to `/Users/adisak.t/Repo/texteditor/_bmad/render/bmad-build/texteditor-dd7e2a0c32cf/c444a9730203448a0ef3/step-02-plan.md` step 6.
...
If a layer needs subagents and you cannot launch them, write the full prompt for each layer under `/Users/adisak.t/Repo/texteditor/_bmad-output/implementation-artifacts` (with placeholders filled in, not just file paths). Stop and ask the user to run each prompt in a separate session and paste back the findings.
...
- **defer** — Everything else: old bugs not caused by this change, ideas for later, groups where every member is `maybe-false` and would be `medium` or `high` if true (record that severity marked unverified, and what would prove it; if it would only be `low`, reject it), or fixes that would edit CLAUDE.md, AGENTS.md, rules, or specs. Add one entry to `/Users/adisak.t/Repo/texteditor/_bmad-output/implementation-artifacts/deferred-work.md`:
...
If `{story_key}` is not empty and `/Users/adisak.t/Repo/texteditor/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `/Users/adisak.t/Repo/texteditor/_bmad/render/bmad-build/texteditor-dd7e2a0c32cf/c444a9730203448a0ef3/sync-sprint-status.md` with `{target_status}` = `review`.
